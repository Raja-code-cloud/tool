"""Regenerate content command handler."""
...
from cloud_content_hub.application.content.commands import RegenerateContentCommand
from cloud_content_hub.application.content.events import ContentRegenerated
...
class RegenerateContentHandler:
    """Orchestrates AI content regeneration."""
    ...
    async def handle(self, actor: ActorContext, command: RegenerateContentCommand) -> OperationDto:
        ...
        async with self._unit_of_work_factory() as unit_of_work:
            ...
            existing = await job_repository.get_by_idempotency_key(
                ...
                job_type="content_regeneration",
                idempotency_key=command.idempotency_key,
            )
            if existing is not None:
                if existing.resource_id != command.request.content_id:
                    raise IdempotencyConflictError(
                        detail="Idempotency key was reused with a different regeneration request.",
                    )
            ...
            asset_id, scope = validate_regeneration_request(
                content, source_version, command.request
            )
            ...
            job = await job_repository.create(
                NewBackgroundJob(
                    ...
                    resource_id=command.request.content_id,
                    idempotency_key=command.idempotency_key,
                    created_by=actor.user_id,
                )
            )
