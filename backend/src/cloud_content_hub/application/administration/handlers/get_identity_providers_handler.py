"""Get identity providers query handler."""
...
from cloud_content_hub.application.administration.queries import GetIdentityProvidersQuery
...
class GetIdentityProvidersHandler:
    """Retrieves identity provider status summaries."""
    ...
    async def handle(
        self,
        actor: ActorContext,
        query: GetIdentityProvidersQuery,
        ...
        providers = await self._provider_health_port.list_providers(
            ProviderHealthCriteria(
                ...
                provider_types=frozenset({ProviderType.IDENTITY}),
