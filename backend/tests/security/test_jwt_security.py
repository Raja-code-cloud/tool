...
from cloud_content_hub.infrastructure.identity.testing.fixtures import identity_factory, identity_settings
from cloud_content_hub.infrastructure.identity.utils import utc_now


@pytest.mark.asyncio
async def test_rejects_expired_access_token(jwt_service) -> None:
    ...
    expired = jwt.encode(
        decoded,
        factory.jwt_service._private_key,
        algorithm="RS256",
        headers={"kid": "default"},
    )
...
@pytest.mark.asyncio
async def test_rejects_wrong_token_type(jwt_service) -> None:
...
@pytest.mark.asyncio
async def test_rejects_untrusted_issuer(jwt_service) -> None:
...
@pytest.mark.asyncio
async def test_rejects_tampered_signature(jwt_service) -> None:
    token = jwt_service.create_access_token("user-1", provider="mock")
    parts = token.split(".")
