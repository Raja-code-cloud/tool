  ...
  return (
    <div
      ...
      className={cn(
        ...
        variant === "info" && "border-info/40 bg-info/10",
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        {children && <div className="text-body text-muted-foreground mt-1">{children}</div>}
...
export function EmptyState({
  ...
  return (
    <div
      className={cn(
        "bg-card grid min-h-64 place-items-center rounded-xl border border-dashed p-8 text-center",
        className,
      )}
      {...props}
    >
      <div className="max-w-md">
        {icon && (
          <div className="bg-muted text-muted-foreground mx-auto mb-4 grid size-12 place-items-center rounded-full [&_svg]:size-5">
            {icon}
          </div>
        )}
...
export type EmptyStateCompositionProps = Omit<EmptyStateProps, "icon"> & { icon?: ReactNode };
export function NoResults({
  icon = <SearchX aria-hidden="true" />,
  ...props
}: EmptyStateCompositionProps): React.JSX.Element {
  ...
  return (
    <div className={cn("grid gap-2", className)} {...props}>
      {Array.from({ length: lines }, (_, index) => (
        <Skeleton key={index} className={cn("h-4", index === lines - 1 && lines > 1 && "w-3/5")} />
      ))}
    </div>
  );
}
...
export function SkeletonCard({
  ...
  return (
    <div className={cn("bg-card rounded-xl border p-5", className)} {...props}>
      {hasMedia && <Skeleton className="mb-4 h-32 w-full" />}
...
export function SkeletonTable({
  ...
  return (
    <div className={cn("overflow-hidden rounded-lg border", className)} {...props}>
      <div className="bg-muted flex gap-4 border-b px-4 py-3">
      ...
      <div className="divide-y">
        {Array.from({ length: rows }, (_, rowIndex) => (
          <div key={rowIndex} className="bg-card flex gap-4 px-4 py-3.5">
...
export function LoadingOverlay({
  ...
  return (
    <div
      ...
      aria-label={label}
      className={cn(
        "bg-background/75 absolute inset-0 z-20 grid place-items-center backdrop-blur-sm",
        className,
      )}
      {...props}
    >
