    ...
    <header className={cn("grid gap-4", className)} {...props}>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          ...
          {description && (
            <p className="max-w-reading text-body text-muted-foreground mt-1.5">{description}</p>
          )}
        </div>
...
export function AppShell({
  ...
  return (
    <div
      className={cn(
        "bg-background text-foreground desktop:grid desktop:grid-cols-[auto_1fr] min-h-dvh",
        className,
      )}
      {...props}
    >
      ...
      <div className="min-w-0">
        ...
        <main
          id="main-content"
          tabIndex={-1}
          className="py-page-mobile tablet:py-page-tablet desktop:py-page-desktop outline-none"
        >
          {children}
        </main>
      </div>
...
export function SkipLink({
  ...
  return (
    <a
      href={href}
      className={cn(
        "bg-primary text-primary-foreground sr-only z-100 rounded-md px-4 py-2 focus:not-sr-only focus:fixed focus:top-2 focus:left-2",
        className,
      )}
      {...props}
    >
...
export function AppHeader({
  ...
  return (
    <header
      className={cn(
        "page-gutter min-h-header bg-background/80 supports-[backdrop-filter]:bg-background/70 sticky top-0 z-20 flex items-center gap-3 border-b backdrop-blur-md",
        className,
      )}
      {...props}
    >
      {leading}
      {title && <p className="text-heading-3 tablet:hidden min-w-0 flex-1 truncate">{title}</p>}
      {search && <div className="tablet:block hidden max-w-xl min-w-0 flex-1">{search}</div>}
