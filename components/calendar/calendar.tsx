    ...
    <div className={cn("relative inline-block", className)}>
      ...
      {isOpen && (
        <div
          role="dialog"
          aria-label={label}
          className="shadow-popover absolute top-full left-0 z-50 mt-2"
        >
          <CalendarSingle
            selected={value}
            onSelect={(date) => {
...
export type CalendarRangeProps = Omit<PropsRange, "mode"> & {
  className?: string;
  selected?: DateRange;
};
...
export type AgendaListProps = HTMLAttributes<HTMLUListElement> & {
  dateLabel: string;
...
export function AgendaList({
  ...
  return (
    <section aria-labelledby="agenda-heading">
      <h3 id="agenda-heading" className="mb-3 font-semibold">
        {dateLabel}
      </h3>
      {items.length === 0 ? (
        ...
        <ul className={cn("grid gap-2", className)} {...props}>
          {items.map((item) => (
            <li
              key={item.id}
              className="bg-card grid grid-cols-[auto_1fr_auto] gap-3 rounded-lg border p-3"
            >
              <time className="text-xs font-semibold tabular-nums">{item.time}</time>
              <div>
                <p className="text-sm font-semibold">{item.title}</p>
                {item.meta && <div className="text-muted-foreground mt-1 text-xs">{item.meta}</div>}
              </div>
              {item.status}
            </li>
          ))}
...
const calendarClassNames = {
  ...
  button_previous:
    "grid size-9 place-items-center rounded-md hover:bg-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring",
  button_next:
    "grid size-9 place-items-center rounded-md hover:bg-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring",
  ...
  day_button:
    "size-10 rounded-md hover:bg-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring",
