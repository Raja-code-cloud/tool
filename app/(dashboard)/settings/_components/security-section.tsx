...
import { ACTIVE_SESSIONS } from "@/constants/settings";
...
export function SecuritySection(): React.JSX.Element {
  ...
  return (
    <SettingsSection id="security" title="Security" description="Protect your account and review where it is signed in.">
      <SettingRows>
        <SettingRow
          ...
          control={
            <div className="flex items-center gap-2">
              {twoFactor && <Badge variant="success"><ShieldCheck className="size-3" aria-hidden="true" />On</Badge>}
        ...
        <SettingRow
          ...
          control={<Switch checked={ssoOnly} onCheckedChange={setSsoOnly} aria-label="Enforce single sign-on" />}
        />
        <SettingRow
          label="Password"
          description="Last changed 3 months ago."
          control={<Button variant="secondary" size="compact">Change password</Button>}
        />
      </SettingRows>
      ...
      <div className="mt-5 border-t pt-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          {otherSessionCount > 0 && (
            <ConfirmationDialog
              trigger={<Button variant="secondary" size="compact">Sign out other sessions</Button>}
            ...
            <li key={session.id} className="flex flex-wrap items-center justify-between gap-2 px-4 py-3">
              ...
              {!session.isCurrent && (
                <Button
                  variant="ghost"
                  size="compact"
                  onClick={() => setSessions((current) => current.filter((entry) => entry.id !== session.id))}
                >
                  Revoke
                </Button>
              )}
