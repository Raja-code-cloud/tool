...
const DANGER_ACTIONS: readonly DangerAction[] = [
  {
    ...
    dialogDescription: "The new owner gains full billing and member control. You will be demoted to admin.",
  ...
  },
  {
    ...
    dialogDescription: "This permanently deletes 128 assets and 6 scheduled posts. This cannot be undone.",
  ...
  },
  {
    ...
    dialogDescription: "All content, members, and connected accounts are permanently removed after a 30-day grace period.",
    ...
    >
      <ul className="divide-y">
        {DANGER_ACTIONS.map((action) => (
          <li key={action.id} className="flex flex-col gap-3 py-3.5 first:pt-0 last:pb-0 tablet:flex-row tablet:items-center tablet:justify-between">
            <div className="min-w-0">
              <p className="text-sm font-semibold">{action.title}</p>
              <p className="mt-1 max-w-prose text-sm text-muted-foreground">{action.description}</p>
            </div>
            <ConfirmationDialog
              trigger={<Button variant="destructive" size="compact" className="shrink-0">{action.buttonLabel}</Button>}
